...
The data structure I plan to use to model my component is an ArrayList. I initially
figured that an array would be a solid way to model a lineup that holds different
players. The nice thing with an ArrayList, though, is that the size of it can be
modified as opposed to creating a whole new one like you would have to do with
an array. Another thing is that if I wanted to add something to the ArrayList,
I could just add it or add it to a specified location. ArrayList also has a lot
of useful methods for implementing my component such as add, remove, size, and
contains.
